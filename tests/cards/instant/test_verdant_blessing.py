from tests.cards.base_env import CardTestCaseBase, load_card_class_from_path
from pycards.creature.Night_Stalker__.model import Night_Stalker__


class TestVerdant_Blessing(CardTestCaseBase):
    async def test_verdant_blessing_puts_land_tapped(self):
        card_cls = load_card_class_from_path("pycards/Instant/Verdant_Blessing/model.py", "Verdant_Blessing")
        env = self.make_env()
        card = card_cls(env.p1)

        from pycards.land.Forest.model import Forest

        env.p1.library = [Forest(env.p1)]
        lands_before = len(env.p1.land_area)
        result = await env.play_card(card, env.p1)
        await env.resolve_stack()

        self.assertTrue(result[0])
        self.assertEqual(len(env.p1.land_area), lands_before + 1)
        self.assertTrue(env.p1.land_area[-1].get_flag("tap"))
        self.assertEqual(env.p1.life, 20)

    async def test_verdant_blessing_no_land_in_library_does_nothing(self):
        card_cls = load_card_class_from_path("pycards/Instant/Verdant_Blessing/model.py", "Verdant_Blessing")
        env = self.make_env()
        card = card_cls(env.p1)

        env.p1.library = [Night_Stalker__(env.p1)]
        lands_before = len(env.p1.land_area)
        lib_before = len(env.p1.library)

        result = await env.play_card(card, env.p1)
        await env.resolve_stack()

        self.assertTrue(result[0])
        self.assertEqual(len(env.p1.land_area), lands_before)
        self.assertEqual(len(env.p1.library), lib_before)

    async def test_verdant_blessing_does_not_add_opponent_lands(self):
        card_cls = load_card_class_from_path("pycards/Instant/Verdant_Blessing/model.py", "Verdant_Blessing")
        from pycards.land.Forest.model import Forest

        env = self.make_env()
        card = card_cls(env.p1)
        env.p1.library = [Forest(env.p1)]
        opp_before = len(env.p2.land_area)
        result = await env.play_card(card, env.p1)
        await env.resolve_stack()
        self.assertTrue(result[0])
        self.assertEqual(len(env.p2.land_area), opp_before)
