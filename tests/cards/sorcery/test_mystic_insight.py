...
class TestMystic_Insight(CardTestCaseBase):
    async def test_mystic_insight_scry_then_draw(self):
        card_cls = load_card_class_from_path("pycards/sorcery/Mystic_Insight/model.py", "Mystic_Insight")
        env = self.make_env()
    ...
    async def test_mystic_insight_sorcery_scrys_two_before_draw(self):
        card_cls = load_card_class_from_path("pycards/sorcery/Mystic_Insight/model.py", "Mystic_Insight")
        env = self.make_env()
        card = card_cls(env.p1)
    ...
    async def test_mystic_insight_does_not_change_opponent_hand_size(self):
        card_cls = load_card_class_from_path("pycards/sorcery/Mystic_Insight/model.py", "Mystic_Insight")
        env = self.make_env()
        card = card_cls(env.p1)
        env.p1.library = [Forest(env.p1)]
