...
class TestIsland(CardTestCaseBase):
    async def test_island_plays_to_land_area(self):
        card_cls = load_card_class_from_path("pycards/land/Island/model.py", "Island")
        env = self.make_env()
    ...
    async def test_island_when_clicked_generates_expected_mana(self):
        card_cls = load_card_class_from_path("pycards/land/Island/model.py", "Island")
        env = self.make_env()
        card = card_cls(env.p1)
    ...
    async def test_island_second_play_adds_another_land(self):
        card_cls = load_card_class_from_path("pycards/land/Island/model.py", "Island")
        env = self.make_env()
        i1, i2 = card_cls(env.p1), card_cls(env.p1)
        await env.play_card(i1, env.p1)
        await env.resolve_stack()
        lands_mid = len(env.p1.land_area)
        await env.play_card(i2, env.p1)
        await env.resolve_stack()
