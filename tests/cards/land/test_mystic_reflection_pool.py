...
class TestMystic_Reflection_Pool(CardTestCaseBase):
    async def test_mystic_reflection_pool_plays_to_land_area(self):
        card_cls = load_card_class_from_path("pycards/land/Mystic_Reflection_Pool/model.py", "Mystic_Reflection_Pool")
        env = self.make_env()
    ...
    async def test_mystic_reflection_pool_when_clicked_generates_expected_mana(self):
        card_cls = load_card_class_from_path("pycards/land/Mystic_Reflection_Pool/model.py", "Mystic_Reflection_Pool")
        env = self.make_env()
        card = card_cls(env.p1)
    ...
    async def test_mystic_reflection_pool_second_play_adds_another_land(self):
        card_cls = load_card_class_from_path("pycards/land/Mystic_Reflection_Pool/model.py", "Mystic_Reflection_Pool")
        env = self.make_env()
    ...
    async def test_mystic_reflection_pool_when_clicked_while_tapped_does_nothing(self):
        card_cls = load_card_class_from_path("pycards/land/Mystic_Reflection_Pool/model.py", "Mystic_Reflection_Pool")
        env = self.make_env()
        card = card_cls(env.p1)
        env.put_on_battlefield(card, env.p1)
