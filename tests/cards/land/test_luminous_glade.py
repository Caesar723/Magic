...
class TestLuminous_Glade(CardTestCaseBase):
    async def test_luminous_glade_plays_to_land_area(self):
        card_cls = load_card_class_from_path("pycards/land/Luminous_Glade/model.py", "Luminous_Glade")
        env = self.make_env()
    ...
    async def test_luminous_glade_when_clicked_generates_expected_mana(self):
        card_cls = load_card_class_from_path("pycards/land/Luminous_Glade/model.py", "Luminous_Glade")
        env = self.make_env()
        card = card_cls(env.p1)
    ...
    async def test_luminous_glade_second_play_adds_another_land(self):
        card_cls = load_card_class_from_path("pycards/land/Luminous_Glade/model.py", "Luminous_Glade")
        env = self.make_env()
    ...
    async def test_luminous_glade_when_clicked_while_tapped_does_nothing(self):
        card_cls = load_card_class_from_path("pycards/land/Luminous_Glade/model.py", "Luminous_Glade")
        env = self.make_env()
        card = card_cls(env.p1)
        env.put_on_battlefield(card, env.p1)
