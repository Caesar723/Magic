...
class TestPlains(CardTestCaseBase):
    async def test_plains_plays_to_land_area(self):
        card_cls = load_card_class_from_path("pycards/land/Plains/model.py", "Plains")
        env = self.make_env()
    ...
    async def test_plains_when_clicked_generates_expected_mana(self):
        card_cls = load_card_class_from_path("pycards/land/Plains/model.py", "Plains")
        env = self.make_env()
        card = card_cls(env.p1)
    ...
    async def test_plains_second_play_adds_another_land(self):
        card_cls = load_card_class_from_path("pycards/land/Plains/model.py", "Plains")
        env = self.make_env()
        p1, p2 = card_cls(env.p1), card_cls(env.p1)
        await env.play_card(p1, env.p1)
        await env.resolve_stack()
        lands_mid = len(env.p1.land_area)
        await env.play_card(p2, env.p1)
        await env.resolve_stack()
        self.assertEqual(len(env.p1.land_area), lands_mid + 1)

    async def test_plains_when_clicked_while_tapped_does_nothing(self):
        card_cls = load_card_class_from_path("pycards/land/Plains/model.py", "Plains")
        env = self.make_env()
        card = card_cls(env.p1)
        env.put_on_battlefield(card, env.p1)
