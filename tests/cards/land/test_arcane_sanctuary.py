...
class TestArcane_Sanctuary(CardTestCaseBase):
    async def test_arcane_sanctuary_plays_to_land_area(self):
        card_cls = load_card_class_from_path("pycards/land/Arcane_Sanctuary/model.py", "Arcane_Sanctuary")
        env = self.make_env()
    ...
    async def test_arcane_sanctuary_when_clicked_generates_expected_mana(self):
        card_cls = load_card_class_from_path("pycards/land/Arcane_Sanctuary/model.py", "Arcane_Sanctuary")
        env = self.make_env()
        card = card_cls(env.p1)
    ...
    async def test_arcane_sanctuary_second_play_adds_another_land(self):
        card_cls = load_card_class_from_path("pycards/land/Arcane_Sanctuary/model.py", "Arcane_Sanctuary")
        env = self.make_env()
    ...
    async def test_arcane_sanctuary_when_clicked_while_tapped_does_nothing(self):
        card_cls = load_card_class_from_path("pycards/land/Arcane_Sanctuary/model.py", "Arcane_Sanctuary")
        env = self.make_env()
        card = card_cls(env.p1)
        env.put_on_battlefield(card, env.p1)
