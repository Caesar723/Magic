...
class Draw_card {
    constructor() {
        ...
        // Add a standalone front texture and a profile here; API names remain unchanged.
        this.art = {
    ...
    style_for(pack) {
        const style = this.art[pack.name];
        const fallback = '/' + String(pack.pack_url || '').replace(/^\/+/, '');
        return {...(style || this.art.Original), image: style ? '/webpages/draw_card/art/packs/' + style.file + '.webp' : fallback, fallback};
    }
