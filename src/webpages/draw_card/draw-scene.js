...
export class DrawScene {
    ...
    buildPack(pack, style) {
        ...
        if (!pack) { this.pack.visible = false; return; }
        const texture = this.texture(style.image, style.fallback);
        const profile = this.profile(style.shape);
        this.packPart(profile.bottom, style, texture, this.pack);
        this.lid = new THREE.Group();
        this.lid.position.set(0, 3.05 * .76, -style.depth / 2);
        this.pack.add(this.lid);
        this.packPart(profile.top, style, texture, this.lid, true);
        // The dark recessed slot stays behind the moving lid.
        const slot = this.solid(new THREE.BoxGeometry(1.86, .035, style.depth * .72), '#263c42', this.pack, false);
        slot.position.set(0, 3.05 * .76 + .008, 0);
        this.seal = new THREE.Group();
        this.pack.add(this.seal);
        ...
        for (const x of [-1.047, 1.047]) {
            for (const y of [.2, 1.96]) {
                const binding = this.solid(new THREE.BoxGeometry(.06, .12, style.depth + .08), style.trim, this.pack);
                binding.position.set(x, y, 0);
            }
        }
        const backRing = this.ring(.59, .028, style.trim, this.pack);
        backRing.position.set(0, 1.2, -style.depth / 2 - .06);
        const backStar = this.solid(new THREE.OctahedronGeometry(.48), style.trim, this.pack);
