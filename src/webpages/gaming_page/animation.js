class Animation{//action
    ...
    // bar_x: Action_Bar.position[0] — rows are drawn at ctx (bar_x-70+80, y) on the table canvas.
    ...
    update_cards(){
        if (this.object_hold instanceof Player){
            ...
            // Preview only: move the *copy*, never the real hand/battle card.
    ...
    // Action-bar history preview builds CardPlane / life-ring meshes on the
    // aux scene. When the pointer leaves the list, hide mode clears
    // `action_showed` but the meshes stayed in the scene — call this there.
