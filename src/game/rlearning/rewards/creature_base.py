...
def get_reward(room:"Base_Agent_Room",agent:"Agent",battled_creature:"Creature"=None,attacker:"Creature"=None):#返回一个评分
    ...
    score_battle_self_creatures=[room.get_creature_reward(card,battled_creature==card)+0.2 for card in agent.battlefield]
    score_battle_self=sum(score_battle_self_creatures)
    
    
    score_battle_oppo_creatures=[room.get_creature_reward(card,card==attacker)+0.1 for card in agent.opponent.battlefield]
