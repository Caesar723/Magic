...
class PPOAgentDataset(BaseDataset):
    ...
    def get_sample(self, data):
        ...
        card_mat=np.zeros((1,40))
        #print(id_dict)
        for i in id_dict:
            id_dict[i]=max(0,min(id_dict[i],4))
            if id_dict[i]==0:continue
            card_mat[0][i-1]=0.25*(id_dict[i])
        #print(card_mat)

        data["card_hand"]["card_matrix"]=card_mat
        action_history_one_hot=np.zeros((self.config["action_history_length"],43))
        ...
        data["self_board"]["slot"]=np.array([i+1 for i in range(10)])
        data["oppo_board"]["slot"]=np.array([i+1 for i in range(10)])

        data["self_board"]["role"]=np.array([1]*10)
        data["oppo_board"]["role"]=np.array([2]*10)

        data["attacker"]["slot"]=np.array([0])
        data["attacker"]["role"]=np.array([0])
    ...
    def collate_state(self,batch,extra_keys=[]):
        ...
        g_keys=[
            ...
            "self_board&card_has_attack","self_board&card_has_defend","self_board&card_mask","self_board&slot","self_board&role",

            "oppo_board&card_special_types","oppo_board&card_atks","oppo_board&card_hps",
            "oppo_board&card_has_attack","oppo_board&card_has_defend","oppo_board&card_mask","oppo_board&slot","oppo_board&role",

            "attacker&card_special_types","attacker&card_atks","attacker&card_hps",
            "attacker&card_has_attack","attacker&card_has_defend","attacker&slot","attacker&role",
        ]
