...
class PPOAgentDataset(BaseDataset):
    ...
    def get_sample(self, data):
        ...
        for i in id_dict:
            ...
            card_mat[0][i-1]=0.5+0.5*(id_dict[i])/4
        #print(card_mat)

        data["card_hand"]["card_matrix"]=card_mat

        combat=np.zeros((10,4))

        if data["attacker"]["card_has_attack"][0] and data["attacker"]["card_has_defend"][0]:
            for i in range(10):
                attacker_atk=data["attacker"]["card_atks"][0]
                attacker_hp=data["attacker"]["card_hps"][0]
                
                if data["self_board"]["card_mask"][i]:
                    card_atk=data["self_board"]["card_atks"][i]
                    card_hp=data["self_board"]["card_hps"][i]
                    
                    combat[i][0]=int(card_atk>=attacker_hp)
                    combat[i][1]=int(attacker_atk>=card_hp)
                    combat[i][2]=int(card_atk>=attacker_hp and attacker_atk>=card_hp)
                    combat[i][3]=int(card_hp>attacker_atk)
            #print(combat)
        data["self_board"]["combat"]=combat
