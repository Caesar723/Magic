...
class PPOTrainer(BaseTrainer):
    ...
    def _forward_independent(self,batch,models,isTrain,step,epoch):
        loss={}
        total_loss=0
        batch=self._predict_independent(batch,models,isTrain,step,epoch)
        
        
        if self.config["w_next_embed_loss"]>0:
            next_embed_loss=F.mse_loss(batch["pre_all_next_embed"],batch["next_all_embed"])
            total_loss+=self.config["w_next_embed_loss"]*next_embed_loss
            loss["next_embed_loss"]=next_embed_loss

        loss["inde_loss"]=total_loss
        return loss
    ...
    def _predict_independent(self,batch,models,isTrain,step,epoch):
        next_batch=self.predict(batch["next_state"],models,isTrain,step,epoch)
        batch["next_all_embed"]=next_batch["all_embed"]

        batch["actions_embed"]=models["ActionEmbed"](batch["action"])

        input_embed=torch.cat([batch["all_embed"],batch["actions_embed"]],dim=-1)
        batch["pre_all_next_embed"]=models["Dense"](input_embed)
        return batch
