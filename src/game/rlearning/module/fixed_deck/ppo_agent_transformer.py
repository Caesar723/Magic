...
class PPOTrainer(BaseTrainer):
    ...
    def predict(self,batch,models,isTrain,step,epoch):
        #print("HeroEmbedSelf")
        hero_embed_self=models["HeroEmbedSelf"](batch["self_life"]).unsqueeze(1)
        #print("HeroEmbedOppo")
        hero_embed_oppo=models["HeroEmbedOppo"](batch["oppo_life"]).unsqueeze(1)
        #print("AttackerEmbed")
        attacker_embed=models["CardsBoardEmbed"](
        ...
        # print("attacker_embed")
        # print(batch["attacker_slot"])
        # print(batch["attacker_role"])
        
        #print("CardsBoardEmbedSelf")
        cards_board_embed_self=models["CardsBoardEmbed"](
            ...
            batch["self_board_slot"],
            batch["self_board_role"],
            # batch["self_board_card_mask"],
        )
        # print("cards_board_embed_self")
        # print(batch["self_board_slot"])
        # print(batch["self_board_role"])
        #print("CardsBoardEmbedOppo")
        cards_board_embed_oppo=models["CardsBoardEmbed"](
            ...
            batch["oppo_board_slot"],
            batch["oppo_board_role"],
            # batch["oppo_board_card_mask"],
        )
        # print("cards_board_embed_oppo")
        # print(batch["oppo_board_slot"])
        # print(batch["oppo_board_role"])

        B=batch["self_board_card_mask"].shape[0]
        device = batch["self_board_card_mask"].device
        pad_mask = torch.cat([
            torch.zeros(B, 3, dtype=torch.bool, device=device),           # HERO_F, HERO_E, ATTACKER 都是有效位 → False
            ~batch["self_board_card_mask"].bool(),                         # [B,10]
            ~batch["oppo_board_card_mask"].bool(),                         # [B,10]
        ], dim=1).contiguous()
        # print("pad_mask")
        # print(pad_mask)
        
        field_embed=torch.cat([
            hero_embed_self,
            hero_embed_oppo,
            attacker_embed,
        ...
        batch["field_embed"]=models["BoardTransformer"](field_embed,pad_mask)
    ...
    def embedding_store(self,state,data):
        if self.embedding_store_data is None:
            self.embedding_store_data={}
        if "field_embed" not in self.embedding_store_data:
            self.embedding_store_data["field_embed"]=[]
        self.embedding_store_data["field_embed"].append({
            "embedding":data["field_embed"].squeeze(0).cpu().numpy(),
            "label":0
        })
